...
function zombie_shootout(zombies, range, ammo) {
  let zombieCount = 0
  for (let i = 0; i <= Math.max(zombies, range, ammo); i++) {
    if (zombieCount === zombies) {
      return `You shot all ${zombies} zombies.`
    }
    if (range === 0) {
      return `You shot ${zombieCount} zombies before being eaten: overwhelmed.`
    }
    if (ammo === 0) {
      return `You shot ${zombieCount} zombies before being eaten: ran out of ammo.`
    }
